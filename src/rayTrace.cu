...
__host__ __device__ RGB getTileColor(float x, float z) {
	int tileWidth = 100;
	int tileHeight = 100;
	
	// Check if < 0 on X or Z axis. Negative values must be shifted.
	if(x <0) x -=tileWidth;
	if(z <0) z -=tileHeight;
	
	// Decide about color
	int projectionX = (int) x / tileWidth;
	int projectionZ = (int) z  / tileHeight;

	if((projectionX + projectionZ)%2 == 0) {
		return RGB(155.0f, 155.0f, 155.0f);
	} else {
		return RGB(130.0f, 30.0f, 30.0f);
	}
...
//
    ...
    if(isPlane){
		RGB planeColor = getTileColor(pInter.x, pInter.z);
        color.r += planeColor.r * phongFactor;
        color.g += planeColor.g * phongFactor;
        color.b += planeColor.b * phongFactor;
    } else {
